...
contract Patient{
    ...
    constructor (string memory _firstName, string memory _lastName, string memory _IID, 
                ...
                string memory _zip, string memory _city, string memory _encryption_key)
    {
        owner = msg.sender;
    ...
    function start_visit(uint _time) public only_owner returns (address){
        address unique_id = address(uint160(uint256(sha256(abi.encodePacked(msg.sender, block.timestamp)))));
    ...
    function getUniqueID(address _unique_id) public view returns (bool, uint256, string memory, uint) {
    return (
        record_mapping[_unique_id].is_uid_generated,
        record_mapping[_unique_id].record_id,
    ...
    function print_record(address _unique_id) public returns (string memory ){
        ...
        require(record_mapping[_unique_id].is_uid_generated == true);
        require(record_mapping[_unique_id].record_status!=1);
        if(msg.sender != record_mapping[_unique_id].patient_address && !doctors[msg.sender] &&!audits[msg.sender]){
            return "You have no authorization.";
        }
    ...
    function doctor_print_record(address _unique_id) public returns (string memory record_status){
        require(record_mapping[_unique_id].is_uid_generated);
        require(doctors[msg.sender]);
        require(record_mapping[_unique_id].record_status!=1);
    ...
    function doctor_query_record(address _unique_id) public returns (string memory){
        string memory result = "Record is queried by doctor/audit.";
        require(record_mapping[_unique_id].is_uid_generated);
        require(doctors[msg.sender]);
        require(record_mapping[_unique_id].record_status!=1);
    ...
    function doctor_copy_record(address _unique_id) public returns (string memory){
        ...
        return "Doctor has successfully copied your medical records";
    }
